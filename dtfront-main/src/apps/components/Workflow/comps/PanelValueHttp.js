...
const PanelValueHttp = forwardRef(({ task }, ref) => {
  ...
  const handleGetData = async () => {
    let result = CloneDeep(HTTP_TaskData);
    ...
    if (IS_NULL(inputData.url)) {
      setError("연산 서버주소를 입력해주세요.");
      return null;
    }
  ...
  const onClickLoadRelationSubModel = () => {
    ...
    let parse = inputData.submodel.replace(":", "/");
  ...
  return (
    <div className="panel-value-wrapper">
      ...
      <div className="row mb-2">
        ...
        <label className="col-form-label col-9">Http</label>
      </div>
      <div className="row mb-2">
      ...
      <div className="row mb-2">
        <label className="col-form-label col-3">
          연산 서버주소 <span className="text-danger">*</span>
        </label>
        <div className="col-9">
          <input
            type="text"
      ...
      <div className="row mb-2">
        ...
        <div className="col-9">
          <input
            ...
            className="form-control"
            name="opId"
            value={inputData ? inputData.opId : ""}
            onChange={handleInputChange}
          ></input>
        </div>
      </div>
