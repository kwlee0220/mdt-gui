...
const SubModelInnerTreeView = ({ instance }) => {
  const { CONVERT_ALLINONE_PICKED_SUBMODEL_TREEDATA } = useConvert();
  ...
  const convertInit = async (data) => {
    const { instance, model } = data;

    let resultlist = await CONVERT_ALLINONE_PICKED_SUBMODEL_TREEDATA(
      instance,
      model
    );
...
export default SubModelInnerTreeView;
