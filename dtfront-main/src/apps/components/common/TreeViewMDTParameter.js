...
const TreeViewMDTParameter = ({ handleSelectedSubmodel }) => {
  const { instanceList } = useCommon();
  const { CONVERT_PARAMETER_TO_TREELIST } = useConvert();

  const [treelist, setTreeList] = useState([]);

  const onClickNode = async (node) => {
    if (node.type === TreeItemType.Parameters) {
  ...
  useEffect(() => {
    if (instanceList) {
      const treelist = CONVERT_PARAMETER_TO_TREELIST(instanceList);
...
export default TreeViewMDTParameter;
